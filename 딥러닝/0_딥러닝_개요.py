...
Image('/content/drive/MyDrive/ComputerProgramming/DeepLearning/2.딥러닝/dl/ai_ml_dl.png')
...
Image('/content/drive/MyDrive/ComputerProgramming/DeepLearning/2.딥러닝/dl/ILSVRC_인식률.png')

Image('/content/drive/MyDrive/ComputerProgramming/DeepLearning/2.딥러닝/dl/ILSVRC_깊이.png')

Image('/content/drive/MyDrive/ComputerProgramming/DeepLearning/2.딥러닝/dl/googLeNet.png')
...
- 단순 퍼셉트론
  - 특징
    ...
    - 입력은 다양하게 
    - 출력은 y가 있다
    - 여러개의 입력에서 출력 y에 수렴하고 싶다
    - 어떤 입력이 출력에 얼마의 영향을 미치는가

- 예시
  - 입사 기념으로 M2 맥북 에어를 구매하려고 한다
    - x1 : 구매할 것인가? => Y OR N
    - x2 : 새 기분으로 일하고 싶다
    - x3 : 딥러닝 하려고 좋은 성능 PC가 필요함
    - x4 : 기존 노트북이 너무 무겁다
    - x10 : 돈이 충분한가?
  - y로 수렴하기 위해 다수결을 적용한다면
    - 다수의 x가 OK라면 구매할 것인가?
      - 다수의 이유가 살 필요가 있어서 사려고 하는데 돈이 부족하다 x10에서 만족 못하는 경우 발생
    - 왜 이런 상황이 발생하는가?
      - 입력 요인별로 가중치가 다르다, 중요도가 다르다
      - 가중치 weight ==> W
    - 식
      - x1 * W1 + x2 * W2 +  .... =
      - 합계 > C(임계값)을 넘어서면 => 구매한다
    - 다양한 입력요인이 보다 올바르게 판단할 수 있다는 것을 증명했음

- 딥러닝의 본질
  - 보다 정확한 예측을 위해 다양한 입력요인들에게 **적절한 W를 계산 하는 과정 => 딥러닝 학습의 본질** => W,b
"""

Image('/content/drive/MyDrive/ComputerProgramming/DeepLearning/2.딥러닝/dl/퍼셉트론.png')

Image('/content/drive/MyDrive/ComputerProgramming/DeepLearning/2.딥러닝/dl/학습의본질.png')