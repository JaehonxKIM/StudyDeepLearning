...
Original file is located at
    https://colab.research.google.com/drive/1fk2OXY4xyr5jKBDdusgDOWb19863evCI
...
Image('/content/drive/MyDrive/ComputerProgramming/DeepLearning/2.딥러닝/res/new_res/GAN-기본구성.png')
...
# 6. 균형을 이루면, 잘 속이지도, 속지도 않는 상태 => 학습종료
...
Image('/content/drive/MyDrive/ComputerProgramming/DeepLearning/2.딥러닝/res/new_res/GAN-1.png')

Image('/content/drive/MyDrive/ComputerProgramming/DeepLearning/2.딥러닝/res/new_res/GAN-2.png')
...
Image('/content/drive/MyDrive/ComputerProgramming/DeepLearning/2.딥러닝/res/new_res/GAN-3.png')

Image('/content/drive/MyDrive/ComputerProgramming/DeepLearning/2.딥러닝/res/new_res/GAN-4.png')

"""# DCGAN

- 데이터 MNIST
- 목표 : DCGAN을 통해서 손글씨 이미지를 생성한다(판별자가 가짜와 진짜를 구분하지 못할 정도까지 훈련시켜서)
"""
...
# 가짜 데이터 생성 => 7*7 => 14*14 => 28*28 => 가짜 이미지 생성
generator = Sequential()
# (?, 100) => (?, 7*7*128)=(?, 6272)
generator.add(Dense(7*7*128, input_dim = 100, activation = LeakyReLU(0.2)))
# 데이터가 흐트려져 잇다 => 층이 깊어도 안정화되게 학습되도록
# 데이터를 조정하는 배치정규화 적용 : 평균 0, 분산 1 => 조정
generator.add(BatchNormalization())
# 세로 7 가로 7 1픽셀의 저보 128
...
d_output = discriminator(generator(g_input)) # 연결해서 판별자 출력
gan = Model(g_input, d_output)               # GAN 모델 완성
gan.compile(loss = 'binary_crossentropy', optimizer = 'adam')
gan.summary()

"""## 훈련

- 학습 진행 간 특정 주기 단위로 가짜데이터를 저장
- 가짜데이터를 뿌리면서 최종적으로 진짜처럼 변화되는 결과물을 확인
"""
